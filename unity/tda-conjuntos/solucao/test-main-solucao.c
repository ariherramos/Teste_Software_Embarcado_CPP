...
void test_conjunto_insere_e_remove(void) {
	unsigned int numeros = rand() % 100000 + 1;
	conjunto_t *a = conjunto_cria();
	TEST_ASSERT_NOT_NULL(a);
	conjunto_inicializa_vazio(a);
	TEST_ASSERT_EQUAL_INT(0, conjunto_numero_elementos(a));
	...
	conjunto_insere_elemento(100, NULL);
	conjunto_remove_elemento(100, NULL);
}

void test_conjunto_uniao(void) {
	int numeros1 = 0;
	int numeros2 = 0;
	conjunto_t *a = conjunto_cria();
	...
	numeros1 = rand() % 1000 + 1;
	numeros2 = rand() % 1000 + 1;
	...
	TEST_ASSERT_EQUAL_INT(1, conjunto_igual(c, d));

	conjunto_remove_elemento(n1[numeros1 / 2], c);
	TEST_ASSERT_EQUAL_INT(0, conjunto_igual(c, d));

	conjunto_uniao(a, b, NULL);
	conjunto_uniao(a, NULL, c);
	conjunto_uniao(NULL, b, c);
...
void test_conjunto_interseccao(void) { 
	int numeros1 = 0;
	int numeros2 = 0;
	conjunto_t *a = conjunto_cria();
	conjunto_t *b = conjunto_cria();
	...
	conjunto_interseccao(a, d, c);
	TEST_ASSERT_EQUAL_INT(1, conjunto_igual(a, c));

	conjunto_interseccao(b, d, c);
	TEST_ASSERT_EQUAL_INT(1, conjunto_igual(b, c));

	conjunto_interseccao(a, d, NULL);
	conjunto_interseccao(a, NULL, c);
	conjunto_interseccao(NULL, d, c);
...
int main(void) { 
	...
	RUN_TEST(test_conjunto_uniao);
	RUN_TEST(test_conjunto_interseccao);
